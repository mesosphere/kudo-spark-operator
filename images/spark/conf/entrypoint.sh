...
spark_submit_conf_params=(--conf "spark.driver.bindAddress=$SPARK_DRIVER_BIND_ADDRESS")

function add_spark_conf_if_non_empty() {
	if [[ -n "$2" ]]; then
		spark_submit_conf_params+=(--conf "$1=$2")
	fi
}
